...
const struct m_sub_options demux_mkv_conf = {
    .opts =
        (const m_option_t[]){
                             {"subtitle-preroll",
             OPT_CHOICE(subtitle_preroll, {"no", 0}, {"yes", 1}, {"index", 2})},
             ...
             OPT_CHOICE(probe_duration, {"no", 0}, {"yes", 1}, {"full", 2})},
...
static int demux_mkv_read_info(demuxer_t *demuxer)
{
    ...
    if (demuxer->params && demuxer->params->matroska_wanted_uids) {
        if (info.n_segment_uid) {
            for (int i = 0; i < demuxer->params->matroska_num_wanted_uids;
                 i++) {
...
static int demux_mkv_open_video(demuxer_t *demuxer, mkv_track_t *track)
{
    ...
    if (!strcmp(track->codec_id,
                "V_MS/VFW/FOURCC")) { /* AVI compatibility mode */
        ...
        switch (t2 == 0x10003000 || t2 == 0x10003001 ? '1'
                                                     : track->codec_id[9]) {
        case '1':
            sh_v->codec = "rv10";
            break;
                               ...
                               int size)
{
    ...
    for (int n = 0; n < num_headers; n++) {
        ...
        if (len > size)
            return;
        if (len > 34 &&
            !memcmp(data, "WAVEFORMATEXTENSIBLE_CHANNEL_MASK=", 34)) {
            char smask[80];
    ...
    if (use_this_block) {
        ...
        for (int i = 0; i < block_info->num_laces; i++) {
            ...
            if (track->dovi_config) {
                ...
                if (av_packet_add_side_data(dp->avpacket, AV_PKT_DATA_DOVI_CONF,
