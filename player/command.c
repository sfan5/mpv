    ...
    for (int n = 0; n < cmd->num_hooks; n++) {
        ...
        if (h->active && strcmp(h->type, type) == 0) {
            if (!mp_client_id_exists(mpctx, h->client_id)) {
                ...
                // Trigger completion of this hook and continue with the next
                // one.
...
static int invoke_hook_handler(struct MPContext *mpctx, struct hook_handler *h)
{
    ...
    *m =
    ...
    if (r < 0) {
        MP_MSG(mpctx,
               mp_client_id_exists(mpctx, h->client_id) ? MSGL_WARN : MSGL_V,
...
static int property_time(int action, void *arg, double time)
{
    ...
    switch (action) {
    case M_PROPERTY_GET: *(double *)arg = time; return M_PROPERTY_OK;
    case M_PROPERTY_GET_TYPE:
    ...
    case M_PROPERTY_KEY_ACTION: {
        ...
        switch (ka->action) {
        case M_PROPERTY_GET: *(double *)ka->arg = time; return M_PROPERTY_OK;
                               ...
                               void *arg)
{
    ...
    switch (action) {
    case M_PROPERTY_GET: *(int *)arg = chapter; return M_PROPERTY_OK;
    case M_PROPERTY_GET_TYPE:
        *(struct m_option *)arg = (struct m_option){
            .type = CONF_TYPE_INT,
            .min  = -1,
                                     ...
                                     int action, void *arg)
{
    ...
    case M_PROPERTY_PRINT: {
        ...
        for (n = 0; n < count; n++) {
            ...
            res =
                talloc_asprintf_append(res, "%s  %s%s\n", time, name,
                               ...
                               void *arg)
{
    ...
    default: return mp_property_generic_option(mpctx, prop, action, arg);
    }
}
...
static void create_hotplug(struct MPContext *mpctx)
{
    ...
    if (!cmd->hotplug) {
        cmd->hotplug =
            ao_hotplug_create(mpctx->global, mp_wakeup_core_cb, mpctx);
    }
}
    ...
    if (action == M_PROPERTY_PRINT) {
        ...
        for (int n = 0; n < list->num_devices; n++) {
            ...
            if (dev->name && name && strcmp(dev->name, name) == 0) {
                *(char **)arg =
                    talloc_strdup(NULL, dev->desc ? dev->desc : "?");
                talloc_free(name);
                return M_PROPERTY_OK;
            }
        }
...
static int property_switch_track(void *ctx, struct m_property *prop, int action,
                                 void *arg)
{
    MPContext *mpctx      = ctx;
    ...
    case M_PROPERTY_PRINT:
        if (track) {
            ...
            *(char **)arg    = talloc_asprintf(
                NULL, "(%d) %s", track->user_tid,
    ...
    case M_PROPERTY_SWITCH: {
        if (mpctx->playback_initialized) {
            ...
            // Simply cycle between "no" and "auto". It's possible that this
            // does not always do what the user means, but keep the complexity
            // low.
...
static int get_track_entry(int item, int action, void *arg, void *ctx)
{
    ...
    struct m_sub_property props[] = {
        ...
        { "demux-bitrate", SUB_PROP_INT(p.bitrate),
          .unavailable = p.bitrate <= 0 },
        { "demux-rotation", SUB_PROP_INT(p.rotate),
          .unavailable = p.rotate <= 0 },
...
{
    res =
        talloc_strdup_append(res, track->selected ? list_current : list_normal);
...
static int property_list_tracks(void *ctx, struct m_property *prop, int action,
                                void *arg)
{
    MPContext *mpctx = ctx;
    if (action == M_PROPERTY_PRINT) {
    ...
    if (action == M_PROPERTY_KEY_ACTION) {
        ...
        if (type != -1) {
            ...
            case M_PROPERTY_PRINT:
                ...
                *(char **)ka->arg = res;
                return M_PROPERTY_OK;
            default: return M_PROPERTY_NOT_IMPLEMENTED;
            }
        }
    }
                                    ...
                                    int action, void *arg)
{
    ...
    return m_property_int_ro(
        action, arg, lrint(get_current_pos_ratio(mpctx, false) * frames));
}
                              ...
                              void *arg)
{
    ...
    if (fmt) {
        ...
        // Alpha type is not supported by FFmpeg, so PL_ALPHA_UNKNOWN may mean
        // alpha is of an unknown type, or simply not present. Normalize to
        // AUTO=no alpha.
    ...
    bool has_hdr10plus =
        pl_hdr_metadata_contains(hdr, PL_HDR_METADATA_HDR10PLUS);
    ...
    struct m_sub_property props[] = {
        ...
        { "aspect-name", SUB_PROP_STR(aspect_name),
          .unavailable = !aspect_name },
        ...
        { "primaries", SUB_PROP_STR(m_opt_choice_str(pl_csp_prim_names,
                                                     p->color.primaries)) },
        { "gamma",
        ...
        { "min-luma", SUB_PROP_FLOAT(hdr->min_luma),
          .unavailable = !has_hdr10 },
        { "max-luma", SUB_PROP_FLOAT(hdr->max_luma),
          .unavailable = !has_hdr10 },
        { "max-cll", SUB_PROP_FLOAT(hdr->max_cll), .unavailable = !has_hdr10 },
        { "max-fall", SUB_PROP_FLOAT(hdr->max_fall),
          .unavailable = !has_hdr10 },
        ...
        { "scene-avg", SUB_PROP_FLOAT(hdr->scene_avg),
          .unavailable = !has_hdr10plus },
        { "max-pq-y", SUB_PROP_FLOAT(hdr->max_pq_y),
          .unavailable = !has_cie_y },
        { "avg-pq-y", SUB_PROP_FLOAT(hdr->avg_pq_y),
          .unavailable = !has_cie_y },
        { 0 }
    };
                                    ...
                                    int action, void *arg)
{
    ...
    if (vo_c->filter->input_params.imgfmt) {
        ...
        struct m_sub_property props[] = { { "w", SUB_PROP_INT(c->disp_w) },
                                        ...
                                        int action, void *arg)
{
    ...
    for (int n = 0; n < f->num_ff_side_data; n++) {
        ...
        if (mpsd->type == AV_FRAME_DATA_GOP_TIMECODE)
            av_timecode_make_mpeg_tc_string(gop_tc,
                                            *(int64_t *)(mpsd->buf->data));
    ...
    if (s12m_tc[0] == '\0' && mpctx->vo_chain) {
        ...
        if (container_fps) {
            ...
            int frame = lrint(get_current_pos_ratio(mpctx, false) *
                              get_frame_count(mpctx));
    ...
    struct m_sub_property props[] = {
        ...
        { "gop-timecode", SUB_PROP_STR(gop_tc),
          .unavailable = gop_tc[0] == '\0' },
                                     ...
                                     int action, void *arg)
{
    ...
    case M_PROPERTY_GET_TYPE:
        *(struct m_option *)arg =
            (struct m_option){ .type = CONF_TYPE_STRING_LIST };
        return M_PROPERTY_OK;
    case M_PROPERTY_GET: {
        char **display_names;
...
static void get_frame_perf(struct mpv_node *node, struct mp_frame_perf *perf)
{
    for (int i = 0; i < perf->count; i++) {
        ...
        for (int n = 0; n < data->count; n++)
            node_array_add(samples, MPV_FORMAT_INT64)->u.int64 =
                data->samples[n];
    }
}
...
static char *asprint_perf(char *res, struct mp_frame_perf *perf)
{
    for (int i = 0; i < perf->count; i++) {
        ...
        res                       = talloc_asprintf_append(
            ...
            (int)pass->last / 1000, (int)pass->avg / 1000,
            (int)pass->peak / 1000);
    }
                                 ...
                                 void *arg)
{
    ...
    // Return early, to avoid having to go through a completely unnecessary
    // VOCTRL
    switch (action) {
    case M_PROPERTY_PRINT:
    case M_PROPERTY_GET: break;
    case M_PROPERTY_GET_TYPE:
        *(struct m_option *)arg = (struct m_option){ .type = CONF_TYPE_NODE };
        return M_PROPERTY_OK;
    default: return M_PROPERTY_NOT_IMPLEMENTED;
                               ...
                               void *arg)
{
    ...
    struct m_sub_property props[] = { { "w", SUB_PROP_INT(vo_res.w) },
                                      ...
                                      { "par",
                                        SUB_PROP_DOUBLE(vo_res.display_par) },
...
static int mp_property_osd_ass(void *ctx, struct m_property *prop, int action,
                               void *arg)
{
    struct m_sub_property props[] = { { "0", SUB_PROP_STR(OSD_ASS_0) },
                                 ...
                                 void *arg)
{
    ...
    struct m_sub_property props[] = { { "w", SUB_PROP_INT(w) },
                                      { "h", SUB_PROP_INT(h) },
                                      { 0 } };
                               ...
                               void *arg)
{
    ...
    if (action == M_PROPERTY_PRINT || action == M_PROPERTY_FIXED_LEN_PRINT) {
        *(char **)arg =
            mp_format_double(NULL, opts->subs_shared->sub_pos[track_ind], 2,
    ...
    if (action == M_PROPERTY_PRINT) {
        ...
        for (int n = 0; n < pl->num_entries; n++) {
            ...
            if (!e->title || p == e->title ||
                mpctx->opts->playlist_entry_name == 1)
            {
                           ...
                           MPContext *mpctx, enum stream_type mt)
{
    if (action == M_PROPERTY_PRINT) {
        struct m_config_option *opt =
            m_config_get_co(mpctx->mconfig, bstr0(prop->name));
        *(char **)arg =
            print_obj_osd_list(*(struct m_obj_settings **)opt->data);
        return M_PROPERTY_OK;
    }
    return mp_property_generic_option(mpctx, prop, action, arg);
}
...
{
    struct m_config_option *opt =
        m_config_get_co(mpctx->mconfig, bstr0(ka->key));
    if (!opt)
        return M_PROPERTY_UNKNOWN;
    if (!opt->data)
        return M_PROPERTY_UNAVAILABLE;
                              ...
                              MPContext *mpctx)
{
    ...
    case M_PROPERTY_GET:
        *(char ***)arg = m_config_list_options(NULL, mpctx->mconfig);
        return M_PROPERTY_OK;
    case M_PROPERTY_KEY_ACTION: return access_options(arg, local, mpctx);
    }
    return M_PROPERTY_NOT_IMPLEMENTED;
}
    ...
    case M_PROPERTY_KEY_ACTION: {
        ...
        if (!cnode) {
            ...
            case M_PROPERTY_SET_NODE: {
                ...
                // otherwise, make it NONE, since we'll be overwriting it at the
                // next level.
                cnode = node_map_badd(
                    node, key,
                    has_split ? MPV_FORMAT_NODE_MAP : MPV_FORMAT_NONE);
                if (!cnode)
                    return M_PROPERTY_ERROR;
                break;
            case M_PROPERTY_GET_TYPE:
                         ...
                         struct clipboard_access_params *params)
{
    ...
    case CLIPBOARD_DATA_TEXT:
        *(char **)arg = talloc_steal(NULL, data.u.text);
        talloc_free(tmp);
        return M_PROPERTY_OK;
    default: talloc_free(tmp); return M_PROPERTY_NOT_IMPLEMENTED;
    }
}
        ...
        case M_PROPERTY_GET_TYPE:
            ...
            case CLIPBOARD_DATA_TEXT:
                *(struct m_option *)act->arg = (struct m_option){
                ...
                return M_PROPERTY_OK;
            default: return M_PROPERTY_UNKNOWN;
            }
        case M_PROPERTY_GET: return get_clipboard(mpctx, act->arg, &params);
        case M_PROPERTY_SET: return set_clipboard(mpctx, act->arg, &params);
        default: return M_PROPERTY_NOT_IMPLEMENTED;
        }
    }
    default: return M_PROPERTY_NOT_IMPLEMENTED;
...
static bool is_property_set(int action, void *val)
{
    ...
    case M_PROPERTY_SET_NODE:
    case M_PROPERTY_MULTIPLY: return true;
    case M_PROPERTY_KEY_ACTION: {
                   ...
                   struct MPContext *ctx)
{
    ...
    if (mp_msg_test(ctx->log, MSGL_V) && is_property_set(action, val)) {
        ...
        void *data         = val;
        switch (action) {
        case M_PROPERTY_SET_NODE: ot.type = &m_option_type_node; break;
        case M_PROPERTY_SET_STRING:
...
static const struct property_osd_display {
    ...
    { "secondary-sub-ass-override",
      "Secondary sub ASS subtitle style override" },
...
static void show_property_osd(MPContext *mpctx, const char *name, int osd_mode)
{
    ...
    if (osd_mode & MP_ON_OSD_MSG) {
        ...
        char *osd_msg =
            talloc_steal(tmp, mp_property_expand_string(mpctx, msg));
...
{
    switch (mediatype) {
    case STREAM_VIDEO: return reinit_video_filters(mpctx) >= 0;
    case STREAM_AUDIO: return reinit_audio_filters(mpctx) >= 0;
    }
    return false;
}
                        ...
                        const char *arg)
{
    ...
    int r =
        m_option_parse(log, co->opt, bstr0(optname), bstr0(arg), &new_chain);
    if (r >= 0)
...
static void recreate_overlays(struct MPContext *mpctx)
{
    ...
    for (int n = 0; n < new->num_parts; n++)
        cmd->overlay_packer->in[n] =
            (struct pos){ new->parts[n].w, new->parts[n].h };
    ...
    for (int n = 0; n < new->num_parts; n++) {
        ...
        void *pdata =
            (uint8_t *)new->packed->planes[0] + pos.y *stride + pos.x * 4;
...
static void cmd_cycle_values(void *p)
{
    ...
    if (first >= cmd->num_args) {
        MP_ERR(mpctx,
               "cycle-values command does not have any value arguments.\n");
        cmd->success = false;
        return;
    }
    ...
    change_property_cmd(cmd, name, M_PROPERTY_SET_STRING,
...
static void continue_cmd_list(struct cmd_list_ctx *list)
{
    while (list->parent->args[0].v.p) {
        ...
        if (sub->flags & MP_ASYNC_CMD) {
            ...
            run_command(list->mpctx, sub, NULL, on_cmd_list_sub_completion,
                        list);
                 ...
                 void *on_completion_priv)
{
    ...
    *ctx                   = (struct mp_cmd_ctx){
    ...
    mp_cmd_dump(mpctx->log, noise ? MSGL_TRACE : MSGL_DEBUG,
                "Run command:", cmd);
...
static void cmd_change_list(void *p)
{
    ...
    int r =
        m_option_parse(mpctx->log, &prop, bstr0(optname), bstr0(value), &val);
    if (r >= 0 && mp_property_do(name, M_PROPERTY_SET, &val, mpctx) <= 0)
        r = -1;
    m_option_free(&prop, &val);
    if (r < 0) {
        set_osd_msg(mpctx, osdl, osd_duration, "Failed setting option: '%s'",
                    name);
        cmd->success = false;
        return;
    }
    ...
    if (sub && refpts != MP_NOPTS_VALUE) {
        ...
        if (sub_control(sub, SD_CTRL_SUB_STEP, a) > 0) {
            if (step) {
                ...
                show_property_osd(
                    mpctx, track_ind == 0 ? "sub-delay" : "secondary-sub-delay",
                    cmd->on_osd);
            } else {
...
static void cmd_expand_text(void *p)
{
    ...
    cmd->result = (mpv_node){ .format   = MPV_FORMAT_STRING,
                              .u.string = mp_property_expand_string(
                                  mpctx, cmd->args[0].v.s) };
}
...
static void cmd_expand_path(void *p)
{
    ...
    cmd->result = (mpv_node){ .format   = MPV_FORMAT_STRING,
                              .u.string = mp_get_user_path(NULL, mpctx->global,
                                                           cmd->args[0].v.s) };
}
...
static void cmd_normalize_path(void *p)
{
    ...
    cmd->result = (mpv_node){
        .format = MPV_FORMAT_STRING,
        .u.string =
            talloc_strdup(NULL, mp_normalize_path(ctx, cmd->args[0].v.s)),
    };
...
{
    switch (action->type) {
    case LOAD_TYPE_INSERT_NEXT: return playlist_get_next(mpctx->playlist, +1);
...
static void cmd_loadlist(void *p)
{
    ...
    struct playlist *pl =
        playlist_parse_file(filename, cmd->abort->cancel, mpctx->global);
    if (pl) {
        ...
        struct playlist_entry *at =
            get_insert_entry(mpctx, &action, insert_at_idx);
        if (at == NULL) {
...
static void cmd_track_reload(void *p)
{
    ...
    if (!nt->lang)
        nt->lang = bstrto0(nt, mp_guess_lang_from_filename(
                                   bstr0(nt->external_filename), NULL));
...
static void cmd_subprocess(void *p)
{
    ...
    node_map_add_flag(res, "killed_by_us",
...
static void cmd_filter_command(void *p)
{
    ...
    cmd->success =
        mp_output_chain_command(chain, cmd->args[0].v.s, &filter_cmd);
}
...
const struct mp_cmd_def mp_cmds[] = {
    ...
    },
    {
        ...
        cmd_revert_seek,
        { { "flags",
            OPT_FLAGS(v.i, { "mark", 2 | 0 }, { "mark-permanent", 2 | 1 }),
            .flags = MP_CMD_OPT_ARG } },
    },
    { "quit",
      cmd_quit,
...

    {
        ...
        cmd_track_add,
        {
            { "url", OPT_STRING(v.s) },
            { "flags",
              OPT_CHOICE(v.i, { "select", 0 }, { "auto", 1 }, { "cached", 2 }),
              .flags = MP_CMD_OPT_ARG },
...

    {
        ...
        {
            { "flags",
              OPT_CHOICE(v.i, { "keep-selection", 1 }, { "reselect", 0 }),
              .flags = MP_CMD_OPT_ARG },
        },
        .spawn_thread          = true,
        .can_abort             = true,
...

    {
        ...
        {
            { "flags",
              OPT_FLAGS(v.i, { "video", 4 | 0 }, { "-", 4 | 0 },
                        { "window", 4 | 1 }, { "subtitles", 4 | 2 },
                        { "each-frame", 8 }),
              OPTDEF_INT(4 | 2) },
            // backwards compatibility
            { "legacy",
              OPT_CHOICE(v.i, { "unused", 0 }, { "single", 0 },
                         { "each-frame", 8 }),
              .flags = MP_CMD_OPT_ARG },
        },
        .spawn_thread = true,
    },
    {
        ...
        cmd_screenshot_to_file,
        {
            { "filename", OPT_STRING(v.s) },
            { "flags",
              OPT_CHOICE(v.i, { "video", 0 }, { "window", 1 },
                         { "subtitles", 2 }),
              OPTDEF_INT(2) },
        },
        .spawn_thread = true,
    ...
    },
    {
        ...
        cmd_loadfile,
        {
            { "url", OPT_STRING(v.s) },
            { "flags",
              OPT_CHOICE(v.i, { "replace", 0 }, { "append", 1 },
                         { "append-play", 2 }, { "insert-next", 3 },
                         { "insert-next-play", 4 }, { "insert-at", 5 },
                         { "insert-at-play", 6 }),
              .flags = MP_CMD_OPT_ARG },
            { "index", OPT_INT(v.i), OPTDEF_INT(-1) },
            { "options", OPT_KEYVALUELIST(v.str_list),
              .flags = MP_CMD_OPT_ARG },
        },
    },
    {
    ...
    { "set",
      cmd_set,
      { { "name", OPT_STRING(v.s) }, { "value", OPT_STRING(v.s) } } },
    ...
    { "flush-status-line",
      cmd_flush_status_line,
      { { "clear", OPT_BOOL(v.b) } } },

    { "notify-property",
      cmd_notify_property,
      { { "property", OPT_STRING(v.s) } } },
    ...
    if (flags & (UPDATE_OSD | UPDATE_SUB_FILT | UPDATE_SUB_HARD)) {
        for (int n = 0; n < num_ptracks[STREAM_SUB]; n++) {
            ...
            if (sub) {
                ...
                if (ret == CONTROL_OK &&
                    flags & (UPDATE_SUB_FILT | UPDATE_SUB_HARD))
                {
