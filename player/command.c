                                  ...
                                  int action, void *arg)
{
    ...
    if (action == M_PROPERTY_PRINT) {
        ...
        for (int n = 0; n < num_editions; n++) {
            ...
            res = talloc_strdup_append(
                res, n == current ? list_current : list_normal);
...
#define ADD_FLAG(ctx, dst, flag, first)                                    \
    do {                                                                   \
...
char *mp_format_track_metadata(void *ctx, struct track *t, bool add_lang)
{
    ...
    if (s && s->codec->disp_w)
        bstr_xappend_asprintf(ctx, &dst, " %dx%d", s->codec->disp_w,
                              s->codec->disp_h);
    ...
    if (s && s->codec->bitrate) {
        bstr_xappend_asprintf(ctx, &dst, " %d kbps",
                              (s->codec->bitrate + 500) / 1000);
    } else if (s && s->hls_bitrate) {
        bstr_xappend_asprintf(ctx, &dst, " %d kbps",
                              (s->hls_bitrate + 500) / 1000);
    }
    bstr_xappend0(ctx, &dst, ")");
                               ...
                               void *arg)
{
    ...
    int track_ind       = *(int *)prop->priv;
    if (action == M_PROPERTY_PRINT) {
        *(char **)arg = talloc_asprintf(NULL, "%4.2f%%/100",
                                        opts->subs_shared->sub_pos[track_ind]);
        return M_PROPERTY_OK;
    }
    return mp_property_generic_option(mpctx, prop, action, arg);
}
                                ...
                                void *arg)
{
    ...
    if (action == M_PROPERTY_PRINT) {
        ...
        for (int n = 0; n < pl->num_entries; n++) {
            ...
            res                      = talloc_strdup_append(
                res, pl->current == e ? list_current : list_normal);
            char *p = e->title;
...
static const struct property_osd_display {
    ...
    {"volume", "Volume",
     .msg         = "Volume: ${?volume:${volume}% "
                    "${?mute==yes:(Muted)}}${!volume:${volume}}", .osd_progbar = OSD_VOLUME, .marker = 100},
    {"volume-gain", "Volume gain",
     .msg         = "Volume gain: ${?volume-gain:${volume-gain} dB "
                    "${?mute==yes:(Muted)}}${!volume-gain:${volume-gain}}", .osd_progbar = OSD_VOLUME, .marker = 0},
    {"ao-volume", "AO Volume",
     .msg         = "AO Volume: ${?ao-volume:${ao-volume}% "
                    "${?ao-mute==yes:(Muted)}}${!ao-volume:${ao-volume}}", .osd_progbar = OSD_VOLUME, .marker = 100},
    {"mute", "Mute"},
    {"ao-mute", "AO Mute"},
...
const struct mp_cmd_def mp_cmds[] = {
    ...
    {"quit",
     cmd_quit, {{"code", OPT_INT(v.i), .flags = MP_CMD_OPT_ARG}},
     .priv = &(const bool){0}},
    {"quit-watch-later",
     cmd_quit, {{"code", OPT_INT(v.i), .flags = MP_CMD_OPT_ARG}},
     .priv = &(const bool){1}},
    {"stop",
     cmd_stop, {{"flags", OPT_FLAGS(v.i, {"keep-playlist", 1}),
       .flags = MP_CMD_OPT_ARG}}},
    ...
    {"print-text",
     cmd_print_text, {{"text", OPT_STRING(v.s)}},
     .is_noisy          = true,
     .allow_auto_repeat = true},
    {"show-text",
     cmd_show_text, {
     ...
     .allow_auto_repeat = true},
    {"expand-text",
     cmd_expand_text, {{"text", OPT_STRING(v.s)}},
     .is_noisy = true},
    {"expand-path",
     cmd_expand_path, {{"text", OPT_STRING(v.s)}},
     .is_noisy = true},
    {"normalize-path", cmd_normalize_path, {{"filename", OPT_STRING(v.s)}}},
    {"escape-ass",
     cmd_escape_ass, {{"text", OPT_STRING(v.s)}},
     .is_noisy = true},
    ...
    {
     "sub-remove", cmd_track_remove,
     {{"id", OPT_INT(v.i), OPTDEF_INT(-1)}},
     .priv = &(const int){STREAM_SUB},
     },
    {
     "audio-remove", cmd_track_remove,
     {{"id", OPT_INT(v.i), OPTDEF_INT(-1)}},
     .priv = &(const int){STREAM_AUDIO},
     },
    {
     "video-remove", cmd_track_remove,
     {{"id", OPT_INT(v.i), OPTDEF_INT(-1)}},
     .priv = &(const int){STREAM_VIDEO},
     },

    {
     "sub-reload", cmd_track_reload,
     {{"id", OPT_INT(v.i), OPTDEF_INT(-1)}},
     .priv                  = &(const int){STREAM_SUB},
     .spawn_thread          = true,
     .can_abort             = true,
...

    {
     ...
     {
            {"flags",
             OPT_FLAGS(v.i, {"video", 4 | 0}, {"-", 4 | 0}, {"window", 4 | 1},
                       {"subtitles", 4 | 2}, {"each-frame", 8}),
             OPTDEF_INT(4 | 2)},
     ...
     },
    {
     "screenshot-to-file", cmd_screenshot_to_file,
     {
            ...
            {"flags",
             OPT_CHOICE(v.i, {"video", 0}, {"window", 1}, {"subtitles", 2}),
             OPTDEF_INT(2)},
        }, .spawn_thread = true,
     },
    {
    ...
    {"playlist-remove",
     cmd_playlist_remove, {
         {"index", OPT_CHOICE(v.i, {"current", -1}), .flags = MP_CMD_OPT_ARG,
          M_RANGE(0, INT_MAX)},
     }},
    {"playlist-move",
     cmd_playlist_move, {
    ...
    {"change-list",
     cmd_change_list, {{"name", OPT_STRING(v.s)},
    ...
    {"multiply",
     cmd_multiply, {{"name", OPT_STRING(v.s)}, {"value", OPT_DOUBLE(v.d)}},
     .allow_auto_repeat = true},
    ...
    {"script-binding",
     cmd_script_binding, {{"name", OPT_STRING(v.s)}},
     .allow_auto_repeat = true,
     .on_updown         = true},

    {"script-message",
     cmd_script_message, {{"args", OPT_STRING(v.s)}},
     .vararg = true},
    {"script-message-to",
     cmd_script_message_to, {{"target", OPT_STRING(v.s)}, {"args", OPT_STRING(v.s)}},
     .vararg = true},
    ...
    {"delete-watch-later-config",
     cmd_delete_watch_later_config, {{"filename", OPT_STRING(v.s), .flags = MP_CMD_OPT_ARG}}},

    {"mouse",
     cmd_mouse, {{"x", OPT_INT(v.i)},
      {"y", OPT_INT(v.i)},
      {"button", OPT_INT(v.i), OPTDEF_INT(-1)},
      {"mode", OPT_CHOICE(v.i, {"single", 0}, {"double", 1}),
       .flags = MP_CMD_OPT_ARG}}},
    {"keybind",
     cmd_key_bind, {{"name", OPT_STRING(v.s)}, {"cmd", OPT_STRING(v.s)}}},
    {"keypress",
     cmd_key, {{"name", OPT_STRING(v.s)}, {"scale", OPT_DOUBLE(v.d), OPTDEF_DOUBLE(1)}},
     .priv = &(const int){0}},
    {"keydown",
     cmd_key, {{"name", OPT_STRING(v.s)}},
     .priv = &(const int){MP_KEY_STATE_DOWN}},
    {"keyup",
     cmd_key, {{"name", OPT_STRING(v.s), .flags = MP_CMD_OPT_ARG}},
    ...
    {
     "ab-loop-dump-cache", cmd_dump_cache_ab,
     {{"filename", OPT_STRING(v.s)}},
     .exec_async = true,
     .can_abort  = true,
     },
